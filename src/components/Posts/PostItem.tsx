...
import { Flex, Icon, Stack, Text , Image, Link, SimpleGrid, Button } from '@chakra-ui/react';
import React, { useEffect, useState } from 'react';
import { AiOutlineDelete, AiFillTags, AiFillLike, AiOutlineLike, AiFillDislike, AiOutlineDislike } from "react-icons/ai";
...
type PostItemProps = {
    ...
    onVote: (post: Post, vote: number, subjectId: string) => void;
        ...
        <Flex
            ...
            {post.imageURLs && (
                post.imageURLs.length > 1 ? (
                    <ul style={{ listStyle: 'none', display: 'flex', justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap' }}>
                        {post.imageURLs.map((imageURL: any, index: number) => {
                            const parts = imageURL.split('.');
                            const extension = parts[parts.length - 1];
                            const orgExtension = extension.split('?');
                            return (
                                <li style={{ listStyle: 'none', margin: '5px' }} key={index}>
                                    <a href={router.pathname !== '/' ? `${imageURL}?single_tab=true` : undefined} target={router.pathname !== '/' ? '_blank' : undefined} rel="noreferrer">
                                        {orgExtension[0] === 'png' || orgExtension[0] === 'jpg' || orgExtension[0] === 'jpeg' ? (
                                            <Image src={imageURL} className="post-image" alt="post image" maxW="100%" maxH="100%" />
                                        ) : orgExtension[0] === 'pdf' ? (
        ...
        <Flex
            ...
            {user
                ...
                <Flex align='center' justify='center'>
                    <Icon as={userVoteValue === 1 ? AiFillLike : AiOutlineLike}
                        color={userVoteValue === 1 ? "#9FB751
                                : "gray.500"}
                        cursor="pointer"
                        onClick={() => onVote(post, userVoteValue === 1 ? 0 : 1, post.id)}
                        w={5}
                        h={5}
                        mr={2}
                    />
                    ...
                    <Icon as={userVoteValue === -1 ? AiFillDislike : AiOutlineDislike}
                        color={userVoteValue === -1 ? "#FF504B" : "gray.500"}
                        cursor="pointer"
                        onClick={() => onVote(post, userVoteValue === -1 ? 0 : -1, post.id)}
                        w={5}
                        h={5}
                        ml={2}
                    />
                    <Text ml={1}>{post.dislikes}</Text>
                    <Icon as={MdOutlineComment} color="gray.500" w={5} h={5} ml={2} />
                    <Text ml={1}>{post.comments}</Text>
                    {userIsCreator && (
                        <Button
                            variant="link"
                            color="red.400"
                ...
                :
                <Flex align='center' justify='center'>
                    <Icon as={AiOutlineLike} color="gray.500" w={5} h={5} mr={2} />
                    <Text>{post.likes}</Text>
                    <Icon as={AiOutlineDislike} color="gray.500" w={5} h={5} ml={2} />
                    <Text ml={1}>{post.dislikes}</Text>
                    <Icon as={MdOutlineComment} color="gray.500" w={5} h={5} ml={2} />
                    <Text ml={1}>{post.comments}</Text>
                </Flex>
            ...
            {homePage && (
                <Flex
                    ...
                    align="center"
                >
                    <Icon as={AiFillTags} color="gray.500" w={5} h={5} />
                    <Text ml={1}>{post.subjectTags.join(', ')}</Text>
                </Flex>
            )}
            ...
            >
                ...
                <Text ml={1} color="#2596be" fontWeight={600}>{highestPercentageName}</Text>
                <Text ml={1} color="#2596be" fontWeight={600}>{highestPercentage}</Text>
            </Flex>
