...
const PostItem:React.FC<PostItemProps> = ({
  ...
  post.imageURLs.length > 1 ? (
    <div style={{ display: 'flex', justifyContent: 'center' }}> {/* Container to center the ul */}
      <ul style={{ listStyle: 'none', display: 'flex', margin: '0 auto', maxWidth: '400px', padding: 0 }}>
        {post.imageURLs.map((imageURL:any) => {
          const parts = imageURL.split('.');
          const extension = parts[parts.length - 1];
          const orgExtension = extension.split('?');
          return (
            <li style={{ listStyle: 'none', margin: '0 auto' }}>
