...
const NewPostForm: React.FC<NewPostFormProps> = ({
  ...
  const handleCreatePost = async () => {
    ...
    if (!isFormValid()) {
      // Show an error message or perform any action to indicate that all fields are required
      return;
    }

    // If all fields are filled, proceed with creating the post
    const newPost: Post = {
  ...
  const isFormValid = () => {
    return (
      textInputs.title &&
      textInputs.body &&
      textInputs.grade.value &&
      textInputs.criteria.value &&
      textInputs.typeOfQuestions.value
    );
  };
