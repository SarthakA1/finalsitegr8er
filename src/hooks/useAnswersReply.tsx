            ...
            if (!existingVote) {
                const answerVoteRef = doc(
                    ...
                    if(user?.uid !== answerReply?.creatorId){
                        const newNotification: Notifications = {
                            ...
                            notification: user?.displayName! || user?.email!.split("@")[0]+' liked your comments reply <a href="/subject/'+answerReply?.subjectId+'/answers/'+answerReply?.postId+'">'+answerReply?.text+'</a>',
                            isRead: 0,
                ...
                if(user?.uid !== answerReply?.creatorId){
                    const newNotification: Notifications = {
                        ...
                        notifyBy: user?.displayName! || user?.email!.split("@")[0],
                        notifyTo: answerReply?.creatorDisplayText!,
                        notification: user?.displayName! || user?.email!.split("@")[0]+' disliked your comments reply <a href="/subject/'+answerReply?.subjectId+'/answers/'+answerReply?.postId+'">'+answerReply?.text+'</a>',
                        isRead: 0,
                        notificationType: 'like-dislike-post',
                        createdAt: serverTimestamp() as Timestamp,
                    }
